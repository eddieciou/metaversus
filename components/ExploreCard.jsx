...
const ExploreCard = ({ id, imgUrl, title, index, active, handleClick }) => (
  <motion.div
    ...
    className={`relative flex items-center justify-center min-w-[170px] h-[700px] transition-[flex] duration-[0.7s] ease-out-flex cursor-pointer ${active === id ? 'lg:flex-[3.5] flex-[10]' : 'lg:flex-[0.5] flex-[2]'}`}
    onClick={() => handleClick(id)}
  >
    <img src={imgUrl} alt="title" className="absolute w-full h-full object-cover rounded-[24px]" />
    { active !== id ? (
      <h3 className="text-white z-0 font-semibold sm:text-[26px] text-[18px] absolute lg:bottom-20 lg:rotate-[-90deg] lg:origin-[0,0]">{title}</h3>
    ) : (
      <div className="absolute bottom-0 p-8 justify-start flex-col w-full bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
        ...
        <h2 className="text-white mt-[24px] font-semibold sm:text-[32px] text-[24px]">{title}</h2>
      </div>
    )}
  </motion.div>
);
